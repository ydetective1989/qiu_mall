 ...
 *
 * $regex = new MongoRegex("/^北京市朝阳区/");
 * 参数：array('username' => $regex)
...
class dbmongo {
     ...
     */
    private $_db = null;
    public  $configFile = '';       //数据库配置路径
    public  $getLastInsId = null;   //返回结果_id
    ...
    */
    public function __construct($configFile){
        ...
        if(!file_exists($configFile)) die("数据库配置文件：".$configFile."不存在!");
        require($configFile);
        if (!empty($username) && !empty($password)) {
            $server = sprintf("mongodb://{$username}:{$password}@{$hostname}:{$port}");
        } else {
            $server = sprintf("mongodb://{$localhost}:{$port}");
        }
        //$server = sprintf("mongodb://%s:%s@%s:%s",$config['port']);
        try {
    ...
    public function insert($colName, $sets, $safe=false, $fsync=false){
        ...
        try {
            ...
            $setarr = $this->_parseArr($sets);
            $this->getLastInsId = $setarr["_id"];
            return true;
     ...
     * @param array $sort 排序字段, array('id'=>1):
     * 表示按id字段升序 array('id'=>-1):表示按id字段降序 array('id'=>1, 'age'=>-1):表示按id升序后再按age降序
     * @param int $limit 取多少条记录
     * @param int $skip 跳过多少条(从多少条开始)
     *
     * @return array
     */
    public function select($colName,$fields=array(),$query=array(),$sort=array(),$limit=0,$skip=0){
    ...
    * @param array $sort 排序字段, array('id'=>1):
    * 表示按id字段升序 array('id'=>-1):表示按id字段降序 array('id'=>1, 'age'=>-1):表示按id升序后再按age降序
    * @param int $nums 取多少条记录
    * @param int $type 分页方式
    *
    * @return array
    */
  	public function fetchPage($colName,$fields=array(),$query=array(),$sort=array(),$nums='20',$type='0')
  	{
        //得到总数量
    		$total = $this->count($colName,$query);
    		//计算分页的偏移量
    		$pageid = isset($_GET['page'])?(int)$_GET['page']:1;
    		$offset = ($pageid-1)*$nums;
    		$offset = ($offset < 0) ? 0 : $offset;  //计算跳过条数
    		//print $query;
        $data['record']	 = $this->select($colName,$fields,$query,$sort,$nums,$offset);
    		require_once(dirname(__FILE__).DIRECTORY_SEPARATOR.'pagenav.php');
    		$pagenav = new pagenav();
    		$data['pages']   = $pagenav->show($total,$nums,$type);
    		$data['total']   = $total;
    		$data['maxpage'] = ceil($total/$nums);  //总页数
    		return $data;
  	}
    ...
    /**
     * 返回指定集合里指定键的不同值的列表(一维数组)
     *
     * @param string $colName 集合名
     * @param array $query 查询条件,具体请看 [查询条件说明文档]
     * @param string $fields 要取其值的字段,默认为 "_id" 字段,类似group
     *
     * @return array
     */

    public function fetchDistinct($colName,$fields="_id",$query=array())
    {
        // 得到集合名
        $col = $this->_getCol($colName);
        // 处理结果集
        $result = $col->distinct($fields,$query);
        return $result;
    }
    ...
    // 指定数据库名称
    private function _getDbv($colName){
