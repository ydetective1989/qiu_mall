...
{
	var name = $("#name").val();
	if(name==""){ msgshow("姓名不能为空，请检查"); return; }
	...
	if(mobile==""){ 
		msgshow("手机号码不能为空，请检查"); return;
	}else{
		if(!isWhiteWpace(mobile)){ msgshow("手机号码中有空格，请检查！"); return; }
		if(mobile.length!=11){ msgshow("手机号码长度不正确，请检查！"); return; }
		if(!isMobile(mobile)){ msgshow("手机号码格式不正确，请检查！"); return; }
	}
	var email = $("#email").val();
	if(email!=""){
		if(!isWhiteWpace(email)){ msgshow("电子邮箱格式中有空格，请检查！"); return; }
		if(!checkEmail(email)){ msgshow("电子邮箱格式不正确，请检查！"); return; }
	}
	$("#editbtn").attr("value","正在提交...");		//锁定按钮
	$("#editbtn").attr("disabled","disabled");		//锁定按钮
	$("#editfrm").submit();
